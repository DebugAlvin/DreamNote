...
+ (NSString*)getTableName
{
    ...
    return [NSStringFromClass(self) stringByReplacingOccurrencesOfString:@"." withString:@""];
}
...
+ (NSString*)getDBImagePathWithName:(NSString*)filename
{
    ...
    NSString* dir = [NSString stringWithFormat:@"dbimg/%@", [NSStringFromClass(self) stringByReplacingOccurrencesOfString:@"." withString:@""]];
...
+ (NSString*)getDBDataPathWithName:(NSString*)filename
{
    ...
    NSString* dir = [NSString stringWithFormat:@"dbdata/%@", [NSStringFromClass(self) stringByReplacingOccurrencesOfString:@"." withString:@""]];
    return [LKDBUtils getPathForDocuments:filename inDir:dir];
}
...
- (NSDictionary*)db_readInfoWithModel:(NSObject*)model class:(Class)clazz
{
    ...
    //[jsonObject setObject:[NSStringFromClass(clazz) stringByReplacingOccurrencesOfString:@"." withString:@""]forKey:LKDB_ClassKey];
...
+ (LKModelInfos*)getModelInfos
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        //oncePropertyDic = [NSMutableDictionary alloc];
    });
    
    LKModelInfos* infos;
    [lock lock];
    
    //infos = [oncePropertyDic objectForKey:[NSStringFromClass(self) stringByReplacingOccurrencesOfString:@"." withString:@""]];
    ...
    if (infos == nil) {
        ...
        //[oncePropertyDic setObject:infos forKey:[NSStringFromClass(self) stringByReplacingOccurrencesOfString:@"." withString:@""]];
        ...
        if ([propertyType hasPrefix:@"T@"]) {
            ...
            if (range.location > 4 && range.location <= propertyType.length) {
                ...
                if ([propertyClassName hasSuffix:@">"]) {
                    ...
                    if (NSClassFromString(propertyClassName) == nil) {
                        NSRange range = [NSStringFromClass(self) rangeOfString: @"."];
                        //}
                        
                        range = NSMakeRange(0, range.location);
                        //propertyClassName = [propertyType substringWithRange:range];
                        propertyClassName = [[[NSStringFromClass(self) substringWithRange:range] stringByAppendingString: @"."] stringByAppendingString:propertyClassName];
                    }
...
- (NSMutableString*)getAllPropertysString
{
    ...
    NSMutableString* sb = [NSMutableString stringWithFormat:@"\n <%@> :\n", [NSStringFromClass(clazz) stringByReplacingOccurrencesOfString:@"." withString:@""]];
